...
public class PathEditor : Editor
{
    ...
    private void DebugDraw()
    {
        ...
        foreach (var n in nodes)
        {
            ...
            { var fmh_48_54_638385371566247804 = Quaternion.identity; Handles.FreeMoveHandle(n.Value - center, 2, Vector2.zero, Handles.CylinderHandleCap); }
        }
    }
    ...
    private void DrawOsm()
    {
        if (OsmReader.Instance.nodes != null)
        {
            ...
            foreach (var n in nodes)
            {
                var fmh_62_58_638385371566254808 = Quaternion.identity; Handles.FreeMoveHandle(n.Value - center, 1f, Vector2.zero, Handles.CylinderHandleCap);
    ...
    private void Draw()
    {
        for (int i = 0; i < path.pathSections.Count; i++)
        {
            ...
            for (int j = 1; j < section.Count; j++)
            {
                ...
                var fmh_97_69_638385371566259990 = Quaternion.identity; Vector2 newPos = Handles.FreeMoveHandle(section[j], .075f, Vector2.zero, Handles.CylinderHandleCap);
